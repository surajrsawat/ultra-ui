...
export const Card = forwardRef<HTMLDivElement, CardProps>(
  (
    ...
    const handleMouseEnter = () => {
      if (hoverable && ref && 'current' in ref) {
        (ref.current as HTMLDivElement).style.boxShadow = shadowElevations[Math.min(elevation + 2, 4)];
      }
    };

    const handleMouseLeave = () => {
      if (hoverable && ref && 'current' in ref) {
        (ref.current as HTMLDivElement).style.boxShadow = shadowElevations[Math.min(elevation, 4)];
      }
    };
